...
unsigned long NAU_ldoRampDelay = 250; //During begin, wait this many millis after configuring the LDO before performing calibrateAFE
...
bool NAU_begin(I2C_HandleTypeDef *hi2c, bool initialize) {
	...
	if (initialize) {
		...
		result &= NAU_setBit(NAU7802_PGA_PWR_PGA_CAP_EN, NAU7802_PGA_PWR); //Enable 330pF decoupling cap on chan 2. From 9.14 application circuit note.

		result &= NAU_clearBit(NAU7802_PGA_LDOMODE, NAU7802_PGA); //Ensure LDOMODE bit is clear - improved accuracy and higher DC gain, with ESR < 1 ohm

		tx_thread_sleep(NAU_ldoRampDelay); //Wait for LDO to stabilize - takes about 200ms

		NAU_getWeight(true, 10, 1000); //Flush

		result &= NAU_calibrateAFE(NAU7802_CALMOD_INTERNAL); //Re-cal analog front end when we change gain, sample rate, or channel
	}
...
bool NAU_available() {
	return (NAU_getBit(NAU7802_PU_CTRL_CR, NAU7802_PU_CTRL));
}
...
void NAU_setLDORampDelay(unsigned long delay) {
	NAU_ldoRampDelay = delay;
}
unsigned long NAU_getLDORampDelay() {
	return NAU_ldoRampDelay;
}
...
void NAU_setZeroOffset(int32_t newZeroOffset) {
	NAU_zeroOffset = newZeroOffset;
}

int32_t NAU_getZeroOffset() {
	return (NAU_zeroOffset);
}
...
void NAU_setCalibrationFactor(float newCalFactor) {
	NAU_calibrationFactor = newCalFactor;
}

float NAU_getCalibrationFactor() {
	return (NAU_calibrationFactor);
}
...
uint8_t NAU_getRegister(uint8_t registerAddress) {
	uint8_t value;
	if (HAL_I2C_Mem_Read(NAU_hi2c, NAU_deviceAddress << 1, registerAddress, I2C_MEMADD_SIZE_8BIT, &value, 1, 1000) != HAL_OK)
		return -1; // Error
	return value;
}
...
bool NAU_setRegister(uint8_t registerAddress, uint8_t value) {
	if (HAL_I2C_Mem_Write(NAU_hi2c, NAU_deviceAddress << 1, registerAddress, I2C_MEMADD_SIZE_8BIT, &value, 1, 1000) != HAL_OK)
		return false; // Error
	return true;
}
...
int32_t NAU_get24BitRegister(uint8_t registerAddress) {
	uint8_t data[3];
	if (HAL_I2C_Mem_Read(NAU_hi2c, NAU_deviceAddress << 1, registerAddress, I2C_MEMADD_SIZE_8BIT, data, 3, 1000) != HAL_OK)
		return 0; // Error
...
bool NAU_set24BitRegister(uint8_t registerAddress, int32_t value) {
	...
	if (HAL_I2C_Mem_Write(NAU_hi2c, NAU_deviceAddress << 1, registerAddress, I2C_MEMADD_SIZE_8BIT, data, 3, 1000) != HAL_OK)
		return false; // Error
...
uint32_t NAU_get32BitRegister(uint8_t registerAddress) {
	uint8_t data[4];
	if (HAL_I2C_Mem_Read(NAU_hi2c, NAU_deviceAddress << 1, registerAddress, I2C_MEMADD_SIZE_8BIT, data, 4, 1000) != HAL_OK)
		return 0; // Error
...
bool NAU_set32BitRegister(uint8_t registerAddress, uint32_t value) {
	...
	if (HAL_I2C_Mem_Write(NAU_hi2c, NAU_deviceAddress << 1, registerAddress, I2C_MEMADD_SIZE_8BIT, data, 4, 1000) != HAL_OK)
		return false; // Error
...
//Helper methods
int32_t NAU_getChannel1Offset() {
	return NAU_get24BitRegister(NAU7802_OCAL1_B2);
}
bool NAU_setChannel1Offset(int32_t value) {
	return NAU_set24BitRegister(NAU7802_OCAL1_B2, value);
}
uint32_t NAU_getChannel1Gain() {
	return NAU_get32BitRegister(NAU7802_GCAL1_B3);
}
bool NAU_setChannel1Gain(uint32_t value) {
	return NAU_set32BitRegister(NAU7802_GCAL1_B3, value);
}
